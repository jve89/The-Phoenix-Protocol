...
const generateTip = async (gender, goalStage) => {
  ...
  const modelsToTry = [
    process.env.GROK_MODEL_PRIMARY || 'grok-4',
    ...(process.env.GROK_MODEL_FALLBACKS || '').split(',').filter(Boolean)
  ];
  ...
  for (const model of modelsToTry) {
    try {
      ...
      await logEvent('content', 'warn', `⚠️ Model ${model} failed for ${variant}: ${err.message}`);
      console.warn(`[generateTip] Model ${model} failed. Trying next...`);
    }
  }
